...
def main():
    ...
    bmr_df = signals.basic_mean_reversion(
    ...
    # Visualise
    plt.figure()
    bmr_df['ClosePrice'].plot(color='blue', lw=3.0, legend=True, grid=True)
    bmr_df['FastEMA'].plot(color='c', lw=1.0, legend=True, grid=True)
    bmr_df['SlowEMA'].plot(color='m', lw=1.0, legend=True, grid=True)
    plt.plot(bmr_df.loc[bmr_df.Trades == 1].index,
             bmr_df.ClosePrice[bmr_df.Trades == 1],
    ...
    plt.figure()
    bmr_df['APO'].plot(color='k', lw=3.0, legend=True, grid=True)
    plt.plot(bmr_df.loc[bmr_df.Trades == 1].index,
             bmr_df.APO[bmr_df.Trades == 1],
             color='r', lw=0, marker='^', markersize=7, label='buy')
    plt.plot(bmr_df.loc[bmr_df.Trades == -1].index,
             bmr_df.APO[bmr_df.Trades == -1],
             color='g', lw=0, marker='v', markersize=7, label='sell')
    ...
    plt.figure()
    bmr_df['Position'].plot(color='k', lw=1.0, legend=True, grid=True)
    plt.plot(bmr_df.loc[bmr_df.Position == 0].index,
             bmr_df.Position[bmr_df.Position == 0],
             color='k', lw=0, marker='.', label='flat')
    plt.plot(bmr_df.loc[bmr_df.Position > 0].index,
             bmr_df.Position[bmr_df.Position > 0],
             color='r', lw=0, marker='+', label='long')
    plt.plot(bmr_df[bmr_df.Position < 0].index,
             bmr_df.Position[bmr_df.Position < 0],
    ...
    plt.figure()
    bmr_df['PnL'].plot(color='k', lw=1.0, legend=True, grid=True)
    plt.plot(bmr_df.loc[bmr_df.PnL > 0].index,
             bmr_df.PnL[bmr_df.PnL > 0],
             color='g', lw=0, marker='.')
    plt.plot(bmr_df.loc[bmr_df.PnL < 0].index,
             bmr_df.PnL[bmr_df.PnL < 0],
             color='r', lw=0, marker='.')
    ...
    goog_data = pd.concat([goog_data, bmr_df], axis=1)
